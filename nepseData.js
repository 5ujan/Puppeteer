...
const scrapeInfo = async (string) => {
  ...
  dataObj = await page.evaluate(() => {
    ...
    const price = document.querySelector(".skyblue-bg h2").innerText.split(" ")[4];
    
    // const tds = Array.from(document.querySelectorAll(".cols-md-5 "))
    const tds =Array.from(document.querySelectorAll(".col-md-5 .text-bold")).map((el)=>el.nextElementSibling.innerText)
    const other =Array.from(document.querySelectorAll(".col-md-4 .text-bold")).map((el)=>el.nextElementSibling.innerText)
    const relevant ={
